...
class App extends React.Component {
  ...
  getPkmn = async (e) => {
    ...
    if (r){
      ...
      var out = filter(natArr, r, legendaries, starters);
...
function filter(nat, r, l, s){ // was reg instead of r
      ...
      "Silvally", "Tapu-Koko", "Tapu-Lele", "Tapu-Bulu", "Tapu-Fini", "Nihilego", "Buzzwole", "Pheromosa", "Xurkitree", "Celesteela", "Kartana", "Guzzlord", "Poipole", "Naganadel", "Stakataka", "Blacephalon", "Cosmog", "Cosmoem", "Solgaleo", "Lunala", "Necrozma", "Magearna", "Marshadow", "Zeraora", "Meltan", "Melmetal", "Kubfu", "Urshifu", "Regieleki", "Regidrago", "Glastrier", "Spectrier", "Zacian", "Zamazenta", "Eternatus", "Calyrex", "Zarude"];
      var starter = ["Venusaur", "Charizard", "Blastoise", "Meganium", "Typhlosion", "Feraligatr", "Sceptile", "Blaziken", "Swampert", "Torterra", "Infernape", "Empoleon", "Serperior", "Emboar", "Samurott", "Chesnaught", "Delphox", "Greninja", "Decidueye", "Incineroar", "Primarina", "Rillaboom", "Cinderace", "Inteleon"];
      var output = [];
      ...
      if (r == 2){
        out = output.slice(0,69)
      } else if (r === 3) {
        out = output.slice(69,123)
      } else if (r === 4) {
        out = output.slice(123,196)
      } else if (r === 5) {
        out = output.slice(196,264)
      } else if (r === 8){
        out = output.slice(264,346)
      } else if (r === 12) {
        out = output.slice(346,384)
      } else if (r === 16) {
        out = output.slice(384,439)
      } else if (r == 25) {
        out = output.slice(439,480)
      }
...
function getNumFromName(x, l, s){
  /* list of names below scraped from wikipedia */
...
function nums(){
  /* list of nums corresponding with final stage evolutions scraped along with names above */
  var num = [3,6,9,12,15,18,20,22,24,26,28,31,34,36,38,40,45,47,49,51,53,55,57,59,62,65,68,71,73,76,78,80,83,85,87,89,91,94,97,99,101,103,105,106,107,110,115,119,121,
          ...
          797,798,799,800,801,802,804,805,806,807,809,812,815,818,820,823,826,828,830,832,834,836,839,841,842,844,845,847,849,851,853,855,858,861,862,863,864,865,
          866,867,869,870,871,873,874,875,876,877,879,880,881,882,883,884,887,888,889,890,892,893,894,895,896,897,898];
  return num;
}
